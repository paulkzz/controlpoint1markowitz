# Ввод названий тикеров выбранных компаний
tickers = input('Пожалуйста,  введите название тикеров компаний через пробел: ')
tickers_list = list(map(str, tickers.split()))

# Загрузка данных по котировкам
import yfinance as yf
data = yf.download(tickers_list, period='12mo')

# Определение и вывод стоимости котировок при закрытии торгов
closeData = data.Close
print(closeData)

# Графики котировок акций каждой компании за 12 месяцев
...
# Ежедневное изменение стоимости котировок акций
dCloseData = closeData.pct_change()
print(dCloseData)

# Средняя годовая доходность каждой акции (252 рабочих дня)
...
# Формирование случайного портфеля акций
import numpy as np
...
# Доходность случайного портфеля
def returnPortf(r):
...
# Риск случайного портфеля (стандартная девиация)
def riskPortf(r):
